...
import type { BodyPartTuning } from './tuning';
...
export function calcHitChance(
  ...
  // Momentum hit bonus for UI preview
  if (opts?.momentum !== undefined && opts.momentum >= 1) {
    raw += 0.05;
  }
...
export function calcDamage(
  ...
  const defs = bodyPartDefs ?? BODY_PART_DEFS;
  const [lo, hi] = defs[bodyPart].damageRange;
